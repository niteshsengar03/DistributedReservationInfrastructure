...
const extendCategoryAvailability = async (categoryData: { roomCategoryId: number, latestDate: Date }): Promise<void> => {
    try {
        ...
        const roomCategory = await getRoomCateogoryById(roomCategoryId);
        if (!roomCategory) {
        ...
        const existingRoom = await findByRoomCategoryIdAndDate(roomCategoryId, nextDate);
        if (existingRoom) {
        ...
        // Add job to queue
        await addRoomGernerationJobToQueue(jobData);
